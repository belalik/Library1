 ...
 * 
 * 1.   Να τυπώνω καλύτερα μηνύματα (με string interpolation (με το $) αντί για concatenation (με το +))
 * 2.   Πιο efficient μεθόδους - αυτή τη στιγμή οι μέθοδοι βγάζουν σωστά αποτελέσματα αλλά δεν είναι αποδοτικές
 * 3.   Να εξηγήσουμε λίγο την διαχείριση ημερομηνίας - με παραδείγματα και documentation της DateTime και άλλων έτοιμων κλάσεων
 * 4.   Να εξηγήσουμε το lambda expression που χρησιμοποιεί η μέθοδος Return (και πώς θα μπορούσαμε να το κάνουμε αλλιώς)   
 * 5.   Πολλές ακόμα αλλαγές στον κώδικα που μπορεί μεν να είναι "σωστός" αλλά μπορεί να γίνει πιο αποδοτικός, κατανοητός κλπ.
 * 
 * 
 */

    /*
     * Ίσως η πιο σημαντική κλάση της άσκησης, αφού εδώ υλοποιείται το μεγαλύτερο μέρος
     * της 'λογικής'.
     */
    class Library
    {
         ...
         *  
         *  *** Σημείωση για μένα todo - Να πούμε στο εργαστήριο για re-usability (πχ το numOfLoans ως παράμετρο, και όχι καρφωτό), 
        ...
        public void LoanItem(User user, Item item, DateTime date)
        {
            ...
            if (item.OnLoan)
            {
                Console.WriteLine($"I am sorry mr.{user.Name} but {item.Title} (ID={item.ItemID}) is currently on loan");
            }
            else
            {
                ...
                {
                    Console.WriteLine("Sorry " + user.Name + " but you already have " + MaxNumberOfLoans + " loans");
                }
                else
                {
                    Console.WriteLine("OK - user " + user.Name + " loaned " + item.Title + " on " + date.ToShortDateString());
         ...
         * υπάρχει τέτοιο Loan.  Δηλαδή να ψάξει αν: 
         * - Όντως υπάρχει ένα τέτοιο Loan μέσα στην λίστα των Loans
         * - Και τότε, αν αυτός που προσπαθεί να επιστρέψει το Item είναι αυτός που το δανείστηκε!
         * 
        ...
        {
            // Αυτή είναι η μέθοδος FindIndex που μπορεί να ψάχνει μια λίστα, και επιστρέφει την θέση του πρώτου αντικειμένου  
            // που ικανοποιεί μια δεδομένη συνθήκη.  Στην προκειμένη περίπτωση, θα ψάξει την λίστα Loans και θα γυρίσει την θέση
            // του πρώτου Loan για το οποίο το ItemLoaned.ItemID θα είναι ίσο με το item.ItemID, με άλλα λόγια θα τσεκάρει αν
            // όντως έχει περαστεί αυτό το Item στη λίστα Loans.

            // Αν δεν βρει κάποιο Loan να ικανοποιεί την συνθήκη, θα γυρίσει τον αριθμό -1. 

            // Θα μπορούσα να φτιάξω δική μου μέθοδο να κάνω αυτήν την αναζήτηση (όπως κάναμε στο παράδειγμα με το Parking
            // στο εργαστήριο), αλλά η c# προσφέρει πολλές τέτοιες
            // έτοιμες μεθόδους που χρησιμοποιούν αυτό που λέμε lambda expression (x => ....). Θα πούμε περισσότερα
            // στο εργαστήριο. 
            int loanIndex = Loans.FindIndex(x => x.ItemLoaned.ItemID == item.ItemID);

            // αν μου γυρίσει -1, σημαίνει πως δεν βρέθηκε Loan
            if (loanIndex == -1)
            {
                Console.WriteLine($"No loan for item {item.Title}, ID={item.ItemID}");
            }

            // αλλιώς όντως υπήρχε το Loan, το παίρνω μέσα από την λίστα Loans
            else
            {
                Loan loan = Loans[loanIndex];

                // Ψάχνω αν ο χρήστης που το επιστρέφει ήταν αυτός που όντως το είχε δανειστεί
                if (user.UserID == loan.UserLoaning.UserID)
                {
                    // Αν ναι, τότε κάνω την επιστροφή
                    Console.WriteLine($"OK, Item {item.Title}, with ID={loan.ItemLoaned.ItemID} is returned by user {user.Name}");
                    loan.ItemLoaned.OnLoan = false;
                    Loans.Remove(loan);
                    user.NumberOfItemsLoaned--;
                }
                // αν όχι, ενημερώνω πως μόνο αυτός που το έχει δανειστεί μπορεί και να το επιστρέψει!
                else
                {
                    Console.WriteLine($"Item {item.Title} is loaned by {loan.UserLoaning.Name} and cannot be returned by {user.Name} ");
                }
            }
        }
        ...
        */
         ...
         * 
         * Επίσης, χρησιμοποιώ τον escape character '\' για να τυπώσω " μέσα στο μήνυμα χωρίς να επηρεάζεται το string. 
         * Απλό παράδειγμα εδώ: https://www.w3schools.com/cs/cs_strings_chars.php
         */
        ...
        /*
         * ΔΕΝ ΧΡΗΣΙΜΟΠΟΙΕΙΤΑΙ ΑΥΤΗ Η ΜΕΘΟΔΟΣ .. ΕΧΕΙ ΑΦΕΘΕΙ ΜΟΝΟ ΓΙΑ ΝΑ ΔΟΥΜΕ ΚΑΠΟΙΕΣ ΛΕΠΤΟΜΕΡΕΙΕΣ ΣΤΟ ΕΡΓΑΣΤΗΡΙΟ
         * 
         * Μέθοδος που επιστρέφει ένα αντικείμενο στην βιβλιοθήκη. 
         * 
         * Παίρνει σαν παραμέτρους έναν χρήστη και ένα Item.  Είναι λίγο δύσκολη - γιατί πρέπει πρώτα να ψάξει αν όντως
         ...
         * ΔΕΝ ΧΡΗΣΙΜΟΠΟΙΕΙΤΑΙ ΑΥΤΗ Η ΜΕΘΟΔΟΣ .. ΕΧΕΙ ΑΦΕΘΕΙ ΜΟΝΟ ΓΙΑ ΝΑ ΔΟΥΜΕ ΚΑΠΟΙΕΣ ΛΕΠΤΟΜΕΡΕΙΕΣ ΣΤΟ ΕΡΓΑΣΤΗΡΙΟ
         */
        public void Return_OLD(User user, Item item)

        {
            // Αυτή είναι η μέθοδος Find που μπορεί να ψάχνει μια λίστα, και επιστρέφει το πρώτο αντικείμενο από την λίστα
